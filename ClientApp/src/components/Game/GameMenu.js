...
const GameMenu = ({selectedCardCount, blackCard, game, whiteCards, hasPlayerSelectedCards, winner, isTurn, playerSelectedCards}) => {
    ...
    const selectCardsBtnClasses = useMemo( () => {
        return Object.entries(blackCard).length > 0 && !hasPlayerSelectedCards && selectedCardCount === blackCard.card.pick ? "" : "cursor-not-allowed opacity-25";
    ...
    const leaveGame = useCallback(() => {
        Swal.fire({
            ...
            if (result.value) {
                ...
                resetCards()(dispatch);
                await gameHub.leaveGame(game.name, user.name);
    ...
    const selectCards = useCallback(() => {
        if (Object.entries(blackCard).length > 0 && selectedCardCount === blackCard.card.pick && !hasPlayerSelectedCards)
        {
            sendSelectCards({
                game,
                userId: user.sub,
