...
const SelectedCards = ({selectedPlayerCards, isTurn}) => {
    ...
    return (
      <div className="w-3/4">
          ...
          <FormGroup className="border-2 border-black rounded mr-2 flex flex-wrap justify-center shadow-inner overflow-auto h-48">
                  {
                      [...pickedCards.keys()].map((userName) => (
                          <FormGroup key={userName} check className="w-5/6 flex self-center border-b-2 border-gray-600 ">
                              {
                                  isTurn && 
                              ...
                              <div className="w-full flex flex-wrap justify-center">
                                  {
                                      pickedCards.get(userName).map(whiteCard => (
                                          <div key={whiteCard.id} className="border-2 rounded bg-white m-1 h-40 p-2 w-1/3" dangerouslySetInnerHTML={{__html: whiteCard.card.text }} />
                                      ))
                                  }
                              </div>
                          </FormGroup>
