...
export const addGame = (user) => {
    return async dispatch => {
        try {
            ...
            await store.getState().gameReducer.gameHub.joinGame(data.name, user.name);
...
export const selectGame = (payload) => {
    return async dispatch => {
        ...
        if (Object.entries(payload.game).length > 0)
        {
            await dispatch(checkUserTurn({
                gameId: payload.game.id,
                userId: payload.user.sub,
            }));
            await store.getState().gameReducer.gameHub.joinGame(payload.game.name, payload.user.name);
...
export const joinGame = (payload) => {
    return async dispatch => {
        try {
            ...
            await store.getState().gameReducer.gameHub.joinGame(data.name, payload.user.name);
            await dispatch(getSelectedPlayerCards({
                gameId: data.id,
            }));
            dispatch(getActivePlayer({
...
export function startGameHubAction(gameHub) {
    return {
        type: gameConstants.START_GAME_HUB,
        gameHub
    }
}

export const startGameHub = (gameHub) => {
    return async dispatch => {
        await gameHub.connect();
        dispatch(startGameHubAction(gameHub));
    }
};